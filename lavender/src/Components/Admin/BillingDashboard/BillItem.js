    ...
    return (
      <li className="list-group-item border-0 d-flex p-4 bg-gray-100 border-radius-lg">
        <div className="d-flex flex-column">
          <h6 className="text-sm">{this.state.customer!==undefined & this.state.customer!==null  && this.state.customer.tenkhachhang}</h6>
          <span className="mb-2 text-xs">
            Số điện thoại:{" "}
            <span className="text-dark font-weight-bold ms-sm-2">
            {this.state.customer!==undefined & this.state.customer!==null  && this.state.customer.sodienthoai}
            </span>
          </span>
          <span className="mb-2 text-xs">
            Email:{" "}
            <span className="text-dark ms-sm-2 font-weight-bold">
            {this.state.customer!==undefined & this.state.customer!==null  && this.state.customer.email}
            </span>
          </span>
          <span className="text-xs">
            Tên sản phẩm: {" "}
            <span className="text-dark ms-sm-2 font-weight-bold">
            {this.state.product!==undefined & this.state.product!==null  && this.state.product.tensanpham}
              {"    "}
            </span>
            {"    "}Đơn giá: {" "}
            <span className="text-dark ms-sm-2 font-weight-bold">
            {this.state.product!==undefined & this.state.product!==null  && this.state.product.dongia}
              {"    "}
            </span>
            {"    "}Số lượng: {" "}
            <span className="text-dark ms-sm-2 font-weight-bold">
